...
##################################################################################
##################" have_a_plot (HP) is a for Tello working librairy #################
...
def refresh_coo(tello):
    global x_list, y_list, z_list

    print(tello.get_current_state())
    if tello.is_flying:
        # get the current state. The fonction "get_current_state()" returns a dict with all fields.4
        x_velo = tello.get_current_state()["x"]
        y_velo = tello.get_current_state()["y"]
        z_velo = tello.get_current_state()["z"]
